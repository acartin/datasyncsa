...
### 4. Construir tablas app `mw_app_product_*_day` despues del ETL

Estado: implementado.

La ruta `/pricing/products/{product_key}` y su drill-down de tienda usan tablas
app normales, no materialized views:

```text
public.mw_app_product_chain_day
public.mw_app_product_store_day
```

Estas tablas reemplazan las materialized views anteriores para evitar que
`pg_restore` reconstruya datos derivados mediante `REFRESH MATERIALIZED VIEW`.
El restore debe cargar datos de estas tablas como tablas normales.

Flujo operativo:

- `services/price-scrapper/commands/build_app_product_history.py` reconstruye
  ambas tablas por `run_key`, `date_key`, rango o bootstrap completo.
- `daily_active_campaigns_analytic_job` ejecuta ese comando despues de
  `load_fact_listing_snapshots` y antes de la validacion final.
- El grano de `mw_app_product_chain_day` es
  cliente/campana/SKU/cadena/dia.
- El grano de `mw_app_product_store_day` es
  cliente/campana/SKU/cadena/tienda/dia.

Notas importantes:

- No reintroducir materialized views para estos contratos del portal.
