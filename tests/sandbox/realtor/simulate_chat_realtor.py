...
Uso:
  python tests/sandbox/realtor/simulate_chat_realtor.py              # modo interactivo
  python tests/sandbox/realtor/simulate_chat_realtor.py --auto       # modo automatico
  python tests/sandbox/realtor/simulate_chat_realtor.py --query "Hola"  # mensaje unico

Compatibilidad:
  python tests/sandbox/simulate_chat_realtor.py  # wrapper legacy
...
AUTO_MESSAGES = [
    "Hola, vi el anuncio de la casa en Heredia y me interesa bastante.",
    "Quiero saber si sigue disponible y en que zona exacta esta ubicada.",
    "Me llamo Lolita Lobales.",
    "Mi correo es lola@zonaplus.com por si me quieres enviar la ficha completa.",
    "Mi telefono es 8888-1234, prefiero que me contacten por WhatsApp.",
    "Busco casa para vivir con mi esposo, mis cinco hijos y dos perros.",
    "Nos interesa minimo 3 habitaciones y patio grande.",
    "Tenemos preaprobacion bancaria con Interfin desde la semana pasada.",
    "Nuestro presupuesto anda entre 95 mil y 120 mil dolares.",
    "Nos gustaria mudarnos pronto, idealmente en menos de dos meses.",
    "Nos importa mucho la seguridad del barrio y que haya escuelas cerca.",
    "Tambien quisiera saber si aceptan negociar un poco el precio.",
    "Podrias confirmarme si la propiedad tiene parqueo para dos carros?",
    "Si todo calza, queremos agendar visita esta misma semana.",
    "Miercoles en la tarde o jueves temprano nos funciona bien.",
    "Si necesitas mas datos para avanzar, te los paso de una vez.",
    "Tambien me interesa conocer los gastos de cierre aproximados.",
    "Quedo atenta para coordinar la cita lo antes posible.",
]
...
class ChatSimulator:
    def __init__(self, client_id: str, base_url: str, model_id: str | None = None, discover_endpoints: bool = False):
        ...
        self.history = []
        self.scorecards = []
        self.message_latencies_ms = []
        self.final_scorecard_wait_ms = None
    ...
    def timed_send_chat(self, query_text: str) -> tuple[dict, float]:
        start = time.perf_counter()
        response = self.send_chat(query_text)
...
""")
    ...
    def run_auto(self):
        ...
        for i, query in enumerate(AUTO_MESSAGES, 1):
            response, latency_ms = self.timed_send_chat(query)
    ...
    def display_summary(self):
        ...
        if self.lead_id:
            ...
            score_wait_start = time.perf_counter()
            latest = self.wait_for_latest_scorecard()
            self.final_scorecard_wait_ms = (time.perf_counter() - score_wait_start) * 1000.0
            if latest:
                ...
                print(f"  wait_scorecard_ms: {self.final_scorecard_wait_ms:.1f}")
        ...
        if self.message_latencies_ms:
            avg_ms = sum(self.message_latencies_ms) / len(self.message_latencies_ms)
            min_ms = min(self.message_latencies_ms)
            max_ms = max(self.message_latencies_ms)
            print("\nLatencia chat por mensaje (API /chat):")
            print(f"  avg_ms: {avg_ms:.1f}")
            print(f"  min_ms: {min_ms:.1f}")
