...
logger = logging.getLogger("inference-core-v3.realtor-search-executor")


class RealtorSearchExecutor:
    """Execute validated realtor SQL generated by the routing/planner layer."""

    def __init__(self, db_session: AsyncSession, search_limit: int | None = None) -> None:
        self.db_session = db_session
        limit = int(search_limit or 4)
        if limit <= 0:
            limit = 4
        self.search_limit = max(1, min(limit, 12))
    ...
    async def execute(
        ...
        *,
        ...
        sql = self._materialize_sql(payload.get("sql") or "", str(client_id), filters)
        search_state = {
        ...
        if intent not in {"PROPERTY_SEARCH", "PROPERTY_INVENTORY", "PROPERTY_PRICE_RANGE"}:
            return {
                ...
                "error": "Unsupported realtor intent",
            }
        ...
        if not sql or not self._validate_sql(sql, str(client_id)):
            ...
            return {
                ...
                "search_state": search_state,
                "error": "Invalid SQL generated by planner",
            }
        try:
            ...
            if intent == "PROPERTY_INVENTORY":
                inventory_sql = sql
                if not re.search(r"(?is)\bSELECT\s+COUNT\s*\(", inventory_sql):
                    inventory_sql = self._build_count_sql(sql) or sql

                rows = await self._run_sql(inventory_sql)
            ...
            visible_components = components[: self.search_limit]
            return {
                "handled": True,
                "status": "results",
                "operation": intent,
            ...
            logger.error("Realtor executor failed for intent=%s: %s", intent, exc)
            return {
                "handled": True,
                "status": "execution_error",
                "operation": intent,
    ...
    def _materialize_sql(self, sql: str, client_id: str, filters: Optional[Dict[str, Any]] = None) -> str:
        ...
        numeric_rewrites = [
            ...
            (
                r"(?i)\(\s*features->>'garage_clean'\s*\)\s*::\s*int\b",
                "COALESCE(NULLIF(regexp_replace(COALESCE(features->>'garage_clean', features->>'garage', ''), '[^0-9\\.]', '', 'g'), '')::numeric, 0)",
            ),
            (
                r"(?i)CAST\(\s*features->>'garage'\s+AS\s+INTEGER\s*\)",
                "COALESCE(NULLIF(regexp_replace(COALESCE(features->>'garage_clean', features->>'garage', ''), '[^0-9\\.]', '', 'g'), '')::numeric, 0)",
            ),
            (
                r"(?i)CAST\(\s*features->>'garage_clean'\s+AS\s+INTEGER\s*\)",
                "COALESCE(NULLIF(regexp_replace(COALESCE(features->>'garage_clean', features->>'garage', ''), '[^0-9\\.]', '', 'g'), '')::numeric, 0)",
            ),
            (
                r"(?i)CAST\(\s*features->>'bedrooms_clean'\s+AS\s+INTEGER\s*\)",
                "COALESCE(NULLIF(regexp_replace(COALESCE(features->>'bedrooms_clean', features->>'bedrooms', ''), '[^0-9\\.]', '', 'g'), '')::numeric, 0)",
            ),
            (
                r"(?i)CAST\(\s*features->>'bedrooms'\s+AS\s+INTEGER\s*\)",
                "COALESCE(NULLIF(regexp_replace(COALESCE(features->>'bedrooms', features->>'bedrooms_clean', ''), '[^0-9\\.]', '', 'g'), '')::numeric, 0)",
            ),
            (
                r"(?i)CAST\(\s*features->>'bathrooms_clean'\s+AS\s+(?:FLOAT|DOUBLE PRECISION|NUMERIC|INTEGER)\s*\)",
                "COALESCE(NULLIF(regexp_replace(COALESCE(features->>'bathrooms_clean', features->>'bathrooms', ''), '[^0-9\\.]', '', 'g'), '')::numeric, 0)",
            ),
            (
                r"(?i)CAST\(\s*features->>'bathrooms'\s+AS\s+(?:FLOAT|DOUBLE PRECISION|NUMERIC|INTEGER)\s*\)",
                "COALESCE(NULLIF(regexp_replace(COALESCE(features->>'bathrooms', features->>'bathrooms_clean', ''), '[^0-9\\.]', '', 'g'), '')::numeric, 0)",
            ),
            (
                r"(?i)CAST\(\s*features->>'sqm_clean'\s+AS\s+INTEGER\s*\)",
                "COALESCE(NULLIF(regexp_replace(COALESCE(features->>'sqm_clean', ''), '[^0-9\\.]', '', 'g'), '')::numeric, 0)",
            ),
        ]
        ...
        pseudo_column_rewrites = [
            ...
            (
                r"(?i)(?<!')\baddress\b(?!')",
                "COALESCE(features->>'address', '')",
            ),
        ]
        for pattern, replacement in pseudo_column_rewrites:
            materialized = re.sub(pattern, replacement, materialized)

        def _property_text_predicate(raw_value: str) -> str:
            safe_value = raw_value.replace("'", "''")
            return f"(title ILIKE '%{safe_value}%')"

        materialized = re.sub(
            r"(?i)\bproperty_type\s*=\s*'([^']+)'",
            lambda match: _property_text_predicate(match.group(1).strip()),
            materialized,
        )
        materialized = re.sub(
            r"(?i)\bproperty_type\s+ILIKE\s+'([^']+)'",
            lambda match: _property_text_predicate(match.group(1).strip().replace("%", "")),
            materialized,
        )
        ...
        if re.search(r"(?i)\bwhere\b", materialized):
            if not re.search(r"(?i)\bprice\s*>\s*0\b", materialized) and not re.search(
                r"(?i)\bcoalesce\(\s*price\s*,\s*0\s*\)\s*>\s*0\b",
                materialized,
            ):
                materialized = re.sub(
                    r"(?i)\bwhere\b",
        ...
        filter_guards: List[str] = []
        property_type = filters.get("property_type")
        if isinstance(property_type, str) and property_type.strip():
            safe_value = property_type.strip().replace("'", "''")
            filter_guards.append(f"title ILIKE '%{safe_value}%'")

        desired_location = filters.get("desired_location")
        if isinstance(desired_location, str) and desired_location.strip():
            safe_value = desired_location.strip().replace("'", "''")
            filter_guards.append(
                "("
                f"title ILIKE '%{safe_value}%' "
                f"OR description ILIKE '%{safe_value}%' "
                f"OR COALESCE(features->>'address', '') ILIKE '%{safe_value}%'"
                ")"
            )

        if filter_guards:
            guard_clause = " AND ".join(filter_guards)
            order_or_limit = re.search(r"(?i)\b(order\s+by|limit)\b", materialized)
            if order_or_limit:
                idx = order_or_limit.start()
                materialized = f"{materialized[:idx]} AND {guard_clause} {materialized[idx:]}"
            else:
                materialized = f"{materialized} AND {guard_clause}"
    ...
    async def _get_images_map(self, property_ids: List[str]) -> Dict[str, str]:
        ...
        try:
            ...
            try:
                ...
                logger.exception("Failed to rollback realtor image lookup error")
            return {}
    ...
    def _build_inventory_details_sql(self, inventory_sql: str) -> str:
        ...
        return (
            "SELECT id, title, description, features, price, public_url "
            f"{tail} "
            "ORDER BY id DESC "
