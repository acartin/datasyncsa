...
logger = logging.getLogger("scoring-core.worker")
...
class ScoringWorker:
    ...
    async def _run_job_payload(
        ...
        if not (vertical_ctx and model_data and prompt_config):
            model_id_raw = job.get("model_id")
            prompt_id_raw = job.get("prompt_id")
            ...
            resolved = await orchestrator.resolve_scoring_context_for_job(
                client_id=client_id,
                conversation_id=conversation_id,
                model_id=model_id,
                prompt_id=prompt_id,
            )
            vertical_ctx = resolved.get("vertical_ctx") or {}
            model_data = resolved.get("model_data") or {}
            prompt_config = resolved.get("prompt_config") or {}

            if not (vertical_ctx and model_data and prompt_config):
                raise ValueError("MISSING_SCORING_CONTEXT")

            snapshot_payload = dict(snapshot)
            snapshot_payload.update(
                orchestrator._build_scoring_context_snapshot(
                    vertical_ctx=vertical_ctx,
                    model_data=model_data,
                    prompt_config=prompt_config,
                    channel=(snapshot.get("scoring_context") or {}).get("channel"),
                )
            )
