"""Lead score store with tenant-prefixed keys."""
...
class LeadStore:
    """Stores lead scores and extracted data independently from the main graph state."""
    ...
    def build_key(client_id: str, session_id: str) -> str:
        return f"{client_id}:session:{session_id}:lead"

    async def get_lead_state(self, client_id: str, session_id: str) -> dict[str, object] | None:
        raw = await self.client.get(self.build_key(client_id, session_id))
        return json.loads(raw) if raw else None

    async def set_lead_state(self, client_id: str, session_id: str, payload: dict[str, object], ttl: int) -> None:
    ...
    async def delete_by_client(self, client_id: str) -> int:
        pattern = f"{client_id}:session:*:lead"
