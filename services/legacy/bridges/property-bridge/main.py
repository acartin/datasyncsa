...
def map_v2_scorecard_to_legacy(scorecard: Dict[str, Any]) -> LeadScoringResult:
    ...
    for item in score_items:
        ...
        # Map runtime criteria to legacy pillars (simplified)
    ...
    return LeadScoringResult(
        ...
        reasoning=scorecard.get("reasoning", "Scoring calculated with runtime model"),
...
async def chat_endpoint(request: PropertyChatRequest):
    ...
    Forwards to the active AI runtime
    ...
    try:
        ...
        async with httpx.AsyncClient(timeout=5.0) as client:
            try:
                ...
                logger.warning(f"Health check failed for AI runtime: {e}")
                inference_status = "unreachable"
        
        return {
            "status": "healthy",
            "service": "property-bridge",
