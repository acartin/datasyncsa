#!/usr/bin/env python3
"""Motor Instaleap configurable para extraer catalogos por cadena y categorias raiz.
...
def runtime_context_from_payload(payload: dict[str, Any]) -> StoreRuntimeContext:
    ...
    if engine != "instaleap":
        raise RuntimeError(
            f"Cadena {chain_id!r} no esta configurada como Instaleap (engine={engine!r})."
        )

    base_url = str(payload.get("base_url") or "").strip()
    if not base_url:
        raise ValueError(f"La config de {chain_id!r} no define base_url.")

    extras = dict(payload.get("engine_extras") or {})

    if "client_id" not in extras or "store_reference" not in extras:
        raise RuntimeError(
            f"engine_extras incompleto para {chain_id!r}. Se requieren client_id y store_reference."
        )
    ...
    config = InstaleapStoreConfig(
        ...
        base_url=base_url,
        pricing_scope=payload.get("pricing_scope") or "default_store_online",
...
def build_chain_scraper(chain_id: str, args: argparse.Namespace) -> InstaleapCatalogScraper:
