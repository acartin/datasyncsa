#!/usr/bin/env python3
"""Transforma stage de catálogo en candidatos y revisiones para snapshots."""
...
def build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Transforma mkt_stage_catalog_item en "
            "mkt_stage_listing_snapshot_candidate y "
...
def main(argv: list[str] | None = None) -> None:
    ...
    rows = fetch_stage_snapshot_source_rows(env, stage_run_keys=stage_run_keys)
    result = build_listing_snapshot_transform_result(rows, stage_run_keys=stage_run_keys)
    write_summary = replace_stage_listing_snapshot_transform_tables(env, result)

    print(
        "Transform stage listing snapshots completado | "
