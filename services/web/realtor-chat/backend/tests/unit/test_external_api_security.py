...
async def test_external_sessions_are_isolated_by_channel_user_id(monkeypatch):
    monkeypatch.setenv("EXTERNAL_API_TOKEN", "token-123")
    monkeypatch.setattr(external_module.feature_flags, "EXTERNAL_API_V1_ENABLED", True, raising=False)
    monkeypatch.setattr(external_module.feature_flags, "SESSION_MULTICHANNEL_ENABLED", True, raising=False)
    ...
    monkeypatch.setattr(external_module.session_manager, "get_session", AsyncMock(return_value={}))
    monkeypatch.setattr(external_module.session_manager, "update_session", AsyncMock(return_value=None))
...
@pytest.mark.asyncio
async def test_external_chat_uses_legacy_session_when_multichannel_flag_off(monkeypatch):
    monkeypatch.setenv("EXTERNAL_API_TOKEN", "token-123")
    monkeypatch.setattr(external_module.feature_flags, "EXTERNAL_API_V1_ENABLED", True, raising=False)
    monkeypatch.setattr(external_module.feature_flags, "SESSION_MULTICHANNEL_ENABLED", False, raising=False)
    _patch_router_transformer(monkeypatch)

    get_session_legacy = AsyncMock(return_value={})
    update_session_legacy = AsyncMock(return_value=None)
    monkeypatch.setattr(external_module.session_manager, "get_session", get_session_legacy)
    monkeypatch.setattr(external_module.session_manager, "update_session", update_session_legacy)
    monkeypatch.setattr(external_module.session_manager, "get_session_multichannel", AsyncMock(return_value={}))
    monkeypatch.setattr(external_module.session_manager, "upsert_session", AsyncMock(return_value=None))
    monkeypatch.setattr(
        external_module.inference_client,
        "chat",
        AsyncMock(return_value={"conversation_id": "conv-legacy", "answer": "ok", "intent": None, "sources": []}),
    )
    ...
    resp = await external_module.external_chat(req, request)

    assert resp.conversation_id == "conv-legacy"
