...
import { getIntradayRadar, getMenu } from "@/lib/api";
...
export default async function IntradayRadarRoute({
  ...
  const [menu, payload] = await Promise.all([getMenu(), getIntradayRadar(filters)]);
  ...
  return (
    <AppShell menu={menu} currentPath={currentPath}>
      <IntradayRadarPage payload={payload} filters={filters} />
    </AppShell>
  );
}
