...
function redirectWithFeedback(campaignId: string, type: "success" | "warning" | "error" | "info", message: string) {
  return redirectTo(`/operations/campaigns/${encodeURIComponent(campaignId)}?tab=stores&${feedbackQuery(type, message)}`);
}

export async function POST(
  request: Request,
  { params }: { params: Promise<{ id: string; locationKey: string }> }
) {
  ...
  const { id, locationKey } = await params;

  const response = await fetch(
    `${API_BASE_URL}/operations/campaigns/${encodeURIComponent(id)}/stores/${encodeURIComponent(locationKey)}`,
    {
      method: "DELETE",
      headers: {
  ...
  return redirectWithFeedback(id, "success", "Campaign store removed successfully.");
}
