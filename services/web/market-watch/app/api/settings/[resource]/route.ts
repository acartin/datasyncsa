...
export async function POST(request: Request, { params }: { params: Promise<{ resource: string }> }) {
  ...
  if (!["users", "roles", "clients"].includes(resource)) {
  ...
  const method = String(payload._method ?? "post").toLowerCase();
  ...
  const targetUrl = method === "patch"
    ? `${API_BASE_URL}/settings/${resource}/${id}`
    : `${API_BASE_URL}/settings/${resource}`;
  ...
  return redirectWithFeedback(resource, "success", method === "patch" ? "Registro actualizado correctamente." : "Registro guardado correctamente.");
}
