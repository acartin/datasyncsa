...
| `services/inference-stack-v2/inference-core-v3/` | `inference-core-v3` | Rebuild si hubo cambios de codigo: `docker compose up -d --build inference-core-v3` | `docker compose exec -T inference-core-v3 python -m py_compile main.py app/**/*.py` (si aplica) |
...
| `services/property-bridge-v2/` | `property-bridge-v2` | Rebuild si cambiaste codigo: `docker compose up -d --build property-bridge-v2` | `docker compose exec -T property-bridge-v2 python -V` (y/o pytest si el servicio incorpora tests) |
| `services/agent-core/` | `agent-core` | Rebuild si cambiaste codigo: `docker compose up -d --build agent-core` | `docker compose exec -T agent-core python -m py_compile main.py app/**/*.py` (si aplica) |
| `services/scoring-core/` | `scoring-core` o `scoring-core-worker` | Rebuild si cambiaste codigo: `docker compose up -d --build scoring-core scoring-core-worker` | `docker compose exec -T scoring-core python -m py_compile main.py app/**/*.py` (si aplica) |
| `services/web/*/frontend/` | No aplica Python (Nginx estatico) | No aplica | Validar con navegador/curl |
| `schemas/` | Contenedores que consumen `schemas` | Reiniciar servicios consumidores tras cambio de contrato | `docker compose restart inference-core-v2 inference-core-v2-worker semantic-adapter-v2 admin-console-api chat-web-renderer-api etl-docs etl-docs-worker` |
